    ...
    func buildHeightCachesAtIndexPathsIfNeeded(indexPaths:[NSIndexPath]){
        ...
        indexPaths.forEach { (indexPath) -> () in
            ...
            if indexPath.row >= rows.count{
                rows.append(ZZTemplateLayoutCellHeightCache.ZZTemplateLayoutCellHeightCacheAbsentValue)
            }
...
extension UITableView{
    ...
    private func zz_precacheIfNeeded(){
        ...
        let mutableIndexPathsToBePrecached = self.zz_allIndexPathsToBePrecached()
